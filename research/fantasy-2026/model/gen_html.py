...
function save(){try{localStorage.setItem('tracker',JSON.stringify(T)); const d=new Date(); $$('[data-t="saved"]').forEach(e=>e.textContent='saved '+d.toTimeString().slice(0,8))}catch(e){$$('[data-t="saved"]').forEach(e=>e.textContent='NOT SAVED: storage blocked')} pushDB()}
// Live sync: when the page runs inside the claude.ai viewer, every save is mirrored to the artifact's shared store (drafts/A, drafts/B) so the draft can be read from outside the browser. Without the viewer this is a no-op.
let DBREF=null,DBQ=null;
function pushDB(){ if(!DBREF) return; clearTimeout(DBQ); DBQ=setTimeout(()=>{ const t=Date.now(); T.__t=t; try{localStorage.setItem('tracker',JSON.stringify(T))}catch(e){} ['A','B'].forEach(lg=>{ const s=state(lg); DBREF.doc('drafts/'+lg).set({picks:s.picks,names:s.names||{},slot:s.slot,updated:t,n:s.picks.length}).then(()=>{$$('[data-t="saved"]').forEach(e=>{if(!/synced/.test(e.textContent)) e.textContent+=' · synced'})}).catch(()=>{}) }) },300) }
(function(){ try{ if(!(window.claude&&typeof window.claude.use==='function')) return; window.claude.use('db').then(db=>{ if(!db) return; DBREF=db; const adopt=(lg,sn)=>{ if(!sn||!sn.exists) return; const v=sn.data(); if(!v||!Array.isArray(v.picks)) return; if((v.updated||0)<=(T.__t||0)) return; const s=state(lg); s.picks.map(p=>p.key).forEach(k=>setDone(lg,k,false)); s.picks=v.picks; s.names=v.names||{}; if(v.slot) s.slot=v.slot; T.__t=v.updated; try{localStorage.setItem('tracker',JSON.stringify(T))}catch(e){} s.picks.forEach(p=>setDone(lg,p.key,true)); render(lg); $$('[data-t="saved"]').forEach(e=>e.textContent='synced from store '+new Date().toTimeString().slice(0,8)) }; Promise.all(['A','B'].map(lg=>db.doc('drafts/'+lg).get().then(sn=>adopt(lg,sn)).catch(()=>{}))).then(()=>{ pushDB(); ['A','B'].forEach(lg=>{ try{ db.doc('drafts/'+lg).onSnapshot(sn=>adopt(lg,sn),()=>{}) }catch(e){} }) }) }).catch(()=>{}) }catch(e){} })();
