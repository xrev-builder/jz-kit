...
(function(){ try{ if(!(window.claude&&typeof window.claude.use==='function')) return; window.claude.use('db').then(db=>{ if(!db) return; DBREF=db; Promise.all(['A','B'].map(lg=>db.doc('drafts/'+lg).get().then(sn=>{ if(!sn||!sn.exists) return; const v=sn.data(); if(!v||!Array.isArray(v.picks)) return; if((v.updated||0)<=(T.__t||0)) return; const s=state(lg); s.picks.map(p=>p.key).forEach(k=>setDone(lg,k,false)); s.picks=v.picks; s.names=v.names||{}; if(v.slot) s.slot=v.slot; T.__t=v.updated; try{localStorage.setItem('tracker',JSON.stringify(T))}catch(e){} s.picks.forEach(p=>setDone(lg,p.key,true)); render(lg) }).catch(()=>{}))).then(()=>pushDB()) }).catch(()=>{}) }catch(e){} })();
